...
import { Search, Upload, FileText, MoreHorizontal, MessageSquare } from 'lucide-react'
...
export default function Library({ hasFiles, onFilesUploaded }: LibraryProps) {
      ...
      {hasFiles ? (
        <>
          ...
          <div className="flex items-center gap-2 px-[206px] pb-4 shrink-0">
            {(['All', 'Images', 'Files'] as Filter[]).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                      ...
                      <td className="py-2">
                        <div className="flex justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                          <button className="flex items-center justify-center w-8 h-8 rounded-lg hover:bg-black/5 transition-colors">
                            <MessageSquare size={16} className="text-[#737373]" />
                          </button>
                        </div>
                      </td>
                      <td className="py-2">
        ...
        <div className="flex-1 overflow-auto px-[206px] pb-8">
          <div className="flex flex-col items-center justify-center min-h-[400px] border-2 border-dashed border-[rgba(115,115,115,0.3)] rounded-[18px]">
            <Upload size={32} className="text-[#737373] mb-4" />
            <button
              onClick={onFilesUploaded}
